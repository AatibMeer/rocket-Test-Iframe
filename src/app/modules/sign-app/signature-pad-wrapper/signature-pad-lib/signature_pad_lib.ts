...
import {IBasicPoint, IOptions, IPointGroup} from '../../../../services/sign-app/signature-pad.interface';
...
export class SignaturePad {
  ...
  _isEmpty: boolean;
  _lastPoints: Point[]; // Stores up to 4 most recent points; used to generate a new curve
  _data: IPointGroup[]; // Stores all points in groups (one group per line or dot)
  _lastVelocity: number;
  _lastWidth: number;
  _strokeMoveUpdate: (event: MouseEvent | Touch) => void;
  ...
  constructor(
    ...
    this.throttle = ('throttle' in options ? options.throttle : 1) as number; // in milisecondss
    this.minDistance = ('minDistance' in options
      ? options.minDistance
      : 1) as number; // in pixels
  ...
  public setScaleRatio(ratio: number) {
    if(ratio == this._scaleRatio) {
      return;
    }
    this._scaleRatio = ratio;
    this.canvas.getContext("2d").restore();
    this.canvas.getContext("2d").scale(this._scaleRatio, this._scaleRatio);
  }

  public on(): void {
    // Disable panning/zooming when touching canvas element
    this.canvas.style.touchAction = 'none';
    (<any>this.canvas.style).msTouchAction = 'none';
  ...
  public off(): void {
    ...
    (<any>this.canvas.style).msTouchAction = 'auto';
  ...
  public fromData(pointGroups: IPointGroup[]): void {
    let pointsCopy = JSON.parse(JSON.stringify(pointGroups));
    this.clear();
    this._fromData(
       pointsCopy,
  ...
  private _createPoint(x: number, y: number): Point {
    ...
    return new Point((x - rect.left) / this._scaleRatio, (y - rect.top) / this._scaleRatio, new Date().getTime());
  }
